...
echo "<div class='row'><div class='col-lg-12'><h3>Access Builds</h3></div></div>";
echo "<div class='row'>";
...
echo "<a href='logAccessBuild.php' class='btn btn-sm btn-outline-primary pull-right' title='Add Access Builds'><i class='fa fa-plus'></i></a>";
echo "<a href='genAccessBuildSummary.php' class='btn btn-outline-success btn-sm pull-right' style='margin-right: 10px;' target='_blank'><i class='fa fa-download'></i> Summary</a>";
echo "</div>";
echo "</div>";
...
foreach($TaskStatusses AS $StatID => $Stat)
{
	if(isset($tmpSt[$StatID]))
	{
		...
		echo "<th><a href='logAccessBuild.php' class='btn btn-sm btn-outline-custom' title='Add Access Builds'><i class='fa fa-plus'></i></a></th></tr>\n";
		$act = '';
		foreach($Tasks AS $TaskID => $Task)
		{
			if($StatID == $Task['task_status'])
			{
				...
				if($Today > substr($Maint['apptime'], 0, 10))
					$HiCls = " class='bg-danger text-white'";
				elseif($Today == substr($Maint['apptime'], 0, 10))
					$HiCls = " class='bg-success text-white'";
				echo "<tr" . $HiCls . ">";
				echo "<td><a href='editAccessBuild.php?t=" . $TaskID . "'>" . $TaskID . "</a></td>";
				echo "<td><a href='editAccessBuild.php?t=" . $TaskID . "'>" . $Task['task_name'] . "</a></td>";
				...
				echo "<td><a href='editAccessBuild.php?t=" . $TaskID . "' class='btn btn-outline-primary btn-sm' title='Edit Task'><i class='fa fa-pencil'></i></a></td>";
				echo "</tr>\n";
			}
		}
		echo "</table>\n";

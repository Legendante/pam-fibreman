...
$ChkGrp = getTaskCheckGroupByName("Maintenance");
...
foreach($TaskFiles AS $FileID => $File)
{
	...
	elseif($File['check_id'] == $PMType['id'])
		$PMPath = $File['filepath'];
	elseif($File['check_id'] == $DBType['id'])
		$DBPath = $File['filepath'];
	elseif($File['check_id'] == $DAType['id'])
		$DAPath = $File['filepath'];
	elseif($File['check_id'] == $SBType['id'])
		$SBPath = $File['filepath'];
	elseif($File['check_id'] == $SAType['id'])
		$SAPath = $File['filepath'];
	elseif($File['check_id'] == $CBType['id'])
		$CBPath = $File['filepath'];
	elseif($File['check_id'] == $CAType['id'])
		$CAPath = $File['filepath'];
	else
		$RemFile[$FileID] = $FileID;
}
...
$objPHPExcel->getProperties()->setDescription("Maintenance build approval for " . $Task['task_name'] . ". Generated by Company");
...
$objPHPActiveSheet->setCellValue('A4', 'Maintenance Build Approval');
...
$PicRow = 95;
$PicCnt = 0;
foreach($RemFile AS $FileID => $FileRec)
{
	if(file_exists($TaskFiles[$FileID]['filepath']))
	{
		$objDrawing = new PHPExcel_Worksheet_Drawing();
		$objDrawing->setName('Photo');
		$objDrawing->setDescription('Photo');
		$objDrawing->setPath($TaskFiles[$FileID]['filepath']);
		$objDrawing->setWidth(255);
		if($PicCnt == 0)
			$objDrawing->setCoordinates('A' . $PicRow);
		else
		{
			$objDrawing->setCoordinates('F' . $PicRow);
			$PicRow += 19;
		}
		$PicCnt = !$PicCnt;
		$objDrawing->setWorksheet($objPHPActiveSheet);

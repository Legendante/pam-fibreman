...
<div class='tab-content'>
	<div class='tab-pane active' id='Proj_1'>
		<form method='POST' action='saveAccessBuild.php' id='projForm'>
...
?>
		...
		<div class='row'><div class='col-md-12'><h4>Access Build Report
		<a href='accessbuildmba.php?t=<?php echo $TaskID; ?>' class='btn btn-outline-success btn-sm float-right'><i class='fa fa-download'></i> Generate MBA</a>
		</h4></div></div>
<?php
$AccBuild = getAccessBuildByTaskID($TaskID);
...
if(count($AccBuild) > 0)
{
	$UnitID = $AccBuild['unitid'];
	$Unit = getComplexeUnitByUnitID($UnitID);
	$ComplexID = $Unit['complexid'];
	// $Complex = getComplexByID($ComplexID);
	$NegChecked[$AccBuild['wascustomerneglect']] = ' selected="selected"';
}
$Files = getComplexFiles($ComplexID);
$AppDate = substr($AccBuild['apptime'], 0, 10);
$AppHour = substr($AccBuild['apptime'], 11, 2);
$AppMins = substr($AccBuild['apptime'], 14, 2);
// echo $AccBuild['apptime'] . " : " . $AppDate . " - " . $AppHour . ":" . $AppMins . "<Br>";
?>
...
?>
		...
		<div class='row'>
			<div class='col-md-1'>Client:</div><div class='col-md-4'>
				<input type='hidden' name='MaintID' id='MaintID' value='<?php echo $AccBuild['id']; ?>'>
		...
		<div class='row'>
			<div class='col-md-1'>Ticket #:</div><div class='col-md-3'><input type='text' name='ticketNum' id='ticketNum' value='<?php echo $AccBuild['ticketnumber']; ?>' class='form-control form-control-sm'></div>
...
<?php
if(count($Files) > 0)
	echo "<button type='button' class='btn btn-outline-success float-right' title='I have files for this complex' data-toggle='modal' data-target='#compfiles-modal'><i class='fa fa-file'></i></button>";
?>
			</div>
		</div>
		<div class='row'>
			...
			<div class='col-md-2'>Resident on site:</div><div class='col-md-4'><input type='text' name='custonsite' id='custonsite' value='<?php echo $AccBuild['custonsite']; ?>' class='form-control form-control-sm'></div>
			
		</div>
		<div class='row'>
			<div class='col-md-1'>Problem:</div><div class='col-md-10'><input type='text' name='problem' id='problem' value='<?php echo $AccBuild['faultdesc']; ?>' class='form-control form-control-sm'></div>
		</div>
		<div class='row'>
			<div class='col-md-1'>Solution:</div><div class='col-md-10'><input type='text' name='solution' id='solution' value='<?php echo $AccBuild['solutiondesc']; ?>' class='form-control form-control-sm'></div>
		</div>
		<div class='row'>
			<div class='col-md-2'>Time on site:</div><div class='col-md-3'><input type='text' name='timeonsite' id='timeonsite' value='<?php echo $AccBuild['timeonsite']; ?>' class='form-control form-control-sm'></div>
...
foreach($TPLocations AS $LocationID => $Location)
{
	...
	if($AccBuild['tplocationid'] == $LocationID)
		echo " selected='selected'";
...
foreach($IFMethods AS $MethodID => $Method)
{
	...
	if($AccBuild['infrastuctid'] == $MethodID)
		echo " selected='selected'";
...
?>
		...
		<div class='row'><div class='col-md-2'>Note:</div><div class='col-md-10'><input type='text' name='maintnote' id='maintnote' value='<?php echo $AccBuild['maintnote']; ?>' class='form-control form-control-sm'></div></div>
		<div class='row'><div class='col-md-12'></div></div>
...
foreach($ItemShow AS $CatID => $Lines)
{
	if($OldCat != $CatID)
	{
		echo "<tr class='table-danger'><th colspan='8'>" . $BOQCats[$CatID]['categoryname'] . "</th></tr>\n";
...
<div class='modal fade' id='compfiles-modal' tabindex='-1' role='dialog' aria-hidden='true'>
	<div class='modal-dialog'>
		<div class='modal-content'>
			<div class='modal-header'><strong>Complex Files</strong><a href="#" class="close" data-dismiss="modal">&times;</a></div>
			<div class='modal-body'>
				<div class='btn-group-vertical'>
<?php

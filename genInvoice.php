...
$FileName = 'uploads/accounting/invoices/Company.Invoice.' . $Client['client_short'] . sprintf("%03d", $NextInvNum) . '.' . date("Ymd") . '.pdf';
...
$TopY += 5;
$pdf->SetXY(100, $TopY);
$pdf->Cell(29, 5, "Reg No. 1234/567890/07", 0);
$TopY += 5;
$pdf->SetXY(100, $TopY);
$pdf->Cell(29, 5, "VAT No. 123456789", 0);
$TopY = 16;
$pdf->SetXY(150, $TopY);
$pdf->Cell(40, 5, "Address,", 0);
$TopY += 5;
$pdf->SetXY(150, $TopY);
...
$TopY += 6;
$pdf->SetXY(15, $TopY);
$pdf->Cell(130, 6, "Account Num: 123456798012345", 0, 0);
$pdf->SetFont('Arial','B',10);
...
$SaveArr['inv_number'] = $NextInvNum;
$SaveArr['invoicesubtotal'] = $SubTotal;
$SaveArr['invoicevattotal'] = $VatOnly;
$SaveArr['invoicetotal'] = $WithVat;
$SaveArr['cittotal'] = $SubTotal * 0.28;
$SaveArr['file_path'] = $FileName;
$InvID = addInvoice($SaveArr);
foreach($ItemArr AS $ItemID => $ItemVal)
{
	$SaveArr = array();
	$SaveArr['invoice_id'] = $InvID;
	$SaveArr['join_id'] = $ItemID;
	$SaveArr['itemtotal'] = $ItemVal;
	addInvoiceItem($SaveArr);
	
	$SaveArr = array();
	$SaveArr['task_status'] = $InvoicedID;
	updateTask($ItemID, $SaveArr);
}
increaseClientBalance($ClientID, $WithVat);
unset($pdf);
